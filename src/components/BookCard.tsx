...
const BookCard = ({ book, size = "md", index = 0 }: BookCardProps) => {
  ...
  return (
    <motion.div
      ...
      className="group flex flex-col gap-2 shrink-0"
    >
      <div className="relative overflow-hidden rounded-lg border-gradient">
        <img
          src={book.cover}
          alt={book.title}
          ...
          loading="lazy"
        />
        <button className="absolute top-2 right-2 rounded-full bg-background/70 backdrop-blur-sm p-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      <div style={{ maxWidth: size === "sm" ? "96px" : "128px" }}>
        ...
        <div className="flex items-center gap-2 mt-0.5">
          ...
          <span className="text-[10px] text-primary font-medium shrink-0">{book.readers}</span>
        </div>
      </div>
    </motion.div>
  );
